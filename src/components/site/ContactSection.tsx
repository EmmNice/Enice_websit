...
/**
 * Guards the `mailto:` href against a malformed address.
 *
 * The field is free text, and a typo would otherwise produce a link that silently does nothing, so
 * anything that is not shaped like an address falls back to the built-in one.
 */
function emailOr(value: string, fallback: string): string {
...
export function ContactSection() {
  // The band's copy, editable through the `home.contact` section.
  const section = useSectionFields("home.contact");
  const email = emailOr(fieldText(section, "email", CORPORATE_EMAIL), CORPORATE_EMAIL);
  // Absent until the section loads, so the form shows by default rather than flashing out of view.
  const showForm = section ? section.showForm !== false : true;
              ...
              <p className="mt-3 max-w-md text-[15px] leading-relaxed text-muted-foreground">
                <StyledText
                  text={fieldText(
                    section,
                    "subheading",
                    "Whether you are looking at product access, an integration, a partnership, or just have a question — send us a message and it reaches us directly.",
                  )}
                />
              </p>
